...
#define VULKAN_API_INSTANCE(proc) \
    ENSURE(vk ## proc = ( PFN_vk ## proc )vkGetInstanceProcAddr( vkCtx_.instance, "vk" #proc ));
#include "vkproc.inl.h"
...
VkDevice vklCreateDevice(vklDeviceSetupProc_t setupProc, void* context)
{
    ...
    ENSURE(vkCreateDevice(gpu, &createInfo, NULL, &device) == VK_SUCCESS);
#define VULKAN_API_DEVICE(proc) \
    ENSURE(vk ## proc = ( PFN_vk ## proc )vkGetDeviceProcAddr( device, "vk" #proc ));
#include "vkproc.inl.h"
    return device;
}
